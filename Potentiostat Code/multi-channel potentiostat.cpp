...
int main() {
    ...
    uint8_t fb_ch1_sel, fb_ch2_sel, fb_ch3_sel, fb_ch4_sel, fb_ch5_sel; // feedback selection number (3 represents largest resistor and 0 the smallest)
    fb_ch1_sel = 2;
    ...
    mux.frequency(8000000);    // set SPI clock frequency
    ...
    {        
        ...
        if (dac_tmp < 1)
        {
            ...
            if (pc.writeable())
            {
                ...
                while ((adc1_tmp < 0.55 && adc1_tmp > 0.45) || (adc1_tmp > 0.95 || adc1_tmp < 0.05))
                {
                    if (adc1_tmp < 0.55 && adc1_tmp > 0.45) // voltage change about fixed reference (0.5) too small; needs amplification
                    {
                        ...
                        fb_ch1_sel++;   // increment feedback selection number (amplify by 1 stage)
                    }
                    else if (adc1_tmp > 0.95 || adc1_tmp < 0.05)  // output saturates (leading to inaccurate estimation); needs attenuation
                    {
                        if (fb_ch1_sel == 0)    // already using smallest feedback path
                            break;  // break out of loop
                        fb_ch1_sel--;   // decrement feedback selection number (attenuate by 1 stage)
                    }
                    ...
                    wait_ms(50);    // need to wait because the feedback path cannot change quick enough before the ADC value is read again
                ...
                // loop to update feedback path for best ADC output (this assumes that the current doesn't change drastically during the loop time)
                while ((adc2_tmp < 0.55 && adc2_tmp > 0.45) || (adc2_tmp > 0.95 || adc2_tmp < 0.05))
                {
                    if (adc2_tmp < 0.55 && adc2_tmp > 0.45) // voltage change about fixed reference (0.5) too small; needs amplification
                    {
                        if (fb_ch2_sel == 3)    // already using largest feedback path
                            break;  // break out of loop
                        fb_ch2_sel++;   // increment feedback selection number (amplify by 1 stage)
                    }
                    else if (adc2_tmp > 0.95 || adc2_tmp < 0.05)    // output saturates (leading to inaccurate estimation); needs attenuation
                    {
                        if (fb_ch2_sel == 0)    // already using smallest feedback path
                            break;  // break out of loop
                        fb_ch2_sel--;   // decrement feedback selection number (attenuate by 1 stage)
                    }
                ...
                // loop to update feedback path for best ADC output (this assumes that the current doesn't change drastically during the loop time)
                while ((adc3_tmp < 0.55 && adc3_tmp > 0.45) || (adc3_tmp > 0.95 || adc3_tmp < 0.05))
                {
                    if (adc3_tmp < 0.55 && adc3_tmp > 0.45) // voltage change about fixed reference (0.5) too small; needs amplification
                    {
                        if (fb_ch3_sel == 3)    // already using largest feedback path
                            break;  // break out of loop
                        fb_ch3_sel++;   // increment feedback selection number (amplify by 1 stage)
                    }
                    else if (adc3_tmp > 0.95 || adc3_tmp < 0.05)    // output saturates (leading to inaccurate estimation); needs attenuation
                    {
                        if (fb_ch3_sel == 0)    // already using smallest feedback path
                            break;  // break out of loop
                        fb_ch3_sel--;   // decrement feedback selection number (attenuate by 1 stage)
                    }
                ...
                // loop to update feedback path for best ADC output (this assumes that the current doesn't change drastically during the loop time)
                while ((adc4_tmp < 0.55 && adc4_tmp > 0.45) || (adc4_tmp > 0.95 || adc4_tmp < 0.05))
                {
                    if (adc4_tmp < 0.55 && adc4_tmp > 0.45) // voltage change about fixed reference (0.5) too small; needs amplification
                    {
                        if (fb_ch4_sel == 3)    // already using largest feedback path
                            break;  // break out of loop
                        fb_ch4_sel++;   // increment feedback selection number (amplify by 1 stage)
                    }
                    else if (adc4_tmp > 0.95 || adc4_tmp < 0.05)    // output saturates (leading to inaccurate estimation); needs attenuation
                    {
                        if (fb_ch4_sel == 0)    // already using smallest feedback path
                            break;  // break out of loop
                        fb_ch4_sel--;   // decrement feedback selection number (attenuate by 1 stage)
                    }
                ...
                // loop to update feedback path for best ADC output (this assumes that the current doesn't change drastically during the loop time)
                while ((adc5_tmp < 0.55 && adc5_tmp > 0.45) || (adc5_tmp > 0.95 || adc5_tmp < 0.05))
                {
                    if (adc5_tmp < 0.55 && adc5_tmp > 0.45) // voltage change about fixed reference (0.5) too small; needs amplification
                    {
                        if (fb_ch5_sel == 3)    // already using largest feedback path
                            break;  // break out of loop
                        fb_ch5_sel++;   // increment feedback selection number (amplify by 1 stage)
                    }
                    else if (adc5_tmp > 0.95 || adc5_tmp < 0.05)    // output saturates (leading to inaccurate estimation); needs attenuation
                    {
                        if (fb_ch5_sel == 0)    // already using smallest feedback path
                            break;  // break out of loop
                        fb_ch5_sel--;   // decrement feedback selection number (attenuate by 1 stage)
                    }
                ...
                myled4 = !myled4;
                //wait(0.5);    // set delay to increase sampling period (and reduce overall number of samples)
            }
            
            elect_sel = elect_sel*2;    // update electrode selection
            if (elect_sel == 0) // reset to 1 after 16-bit overflow
                elect_sel = 1;
        }
    }
